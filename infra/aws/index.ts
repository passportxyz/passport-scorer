...
// Cost plan (internal-docs#518 / #1230): the read_replica_analytics dump tasks were
// removed — daily-data-dump (fed only the now-deleted Redshift loads), the OSO export
// (no commitment), and the Gitcoin/Allo grants exports (no live grants round; the
// community-335 UI score is served from the primary DB, not these dumps). This unblocks
// dropping the analytics-read replica. See passportxyz/core-infra for the replica/Redshift deletes.
...
const appApiResult = createAppApiLambdaFunctions({
  ...
  bucketId: codeBucketId,
  passportXyzDomainName: passportXyzDomainName,
});
