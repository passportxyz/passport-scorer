...
export function createLambdaFunction(
  ...
  new aws.iam.RolePolicyAttachment(
    `${functionArgs.name}-log-policy-attachment`,
    {
  ...
  new aws.iam.RolePolicyAttachment(
    `${functionArgs.name}-vpc-policy-attachment`,
    {
  ...
  if (secretManagerArns.length > 0) {
    pulumi.all(secretManagerArns).apply((secretManagerArnsStrings) => {
      const secretPolicy = new aws.iam.Policy(
        `${functionArgs.name}-secret-policy`,
        {
      ...
      new aws.iam.RolePolicyAttachment(
        `${functionArgs.name}-secret-policy-attachment`,
        {
