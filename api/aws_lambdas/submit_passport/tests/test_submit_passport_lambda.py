...
def make_test_event_scorer_id(api_key, address, scorer_id):
    """
    Creates a mock event for testing purposes.
    """
    return {
        ...
        "body": json.dumps({"address": address, "scorer_id": scorer_id}),
        "isBase64Encoded": False,
    }
...
def test_successful_authentication_and_numeric_scorer_id(
    scorer_api_key,
    scorer_community_with_binary_scorer,
    passport_holder_addresses,
    mocker,
    ...
    mocker.patch("registry.atasks.validate_credential", side_effect=[[], [], []])
    address = passport_holder_addresses[0]["address"].lower()
    event = make_test_event_scorer_id(
        scorer_api_key, address, int(scorer_community_with_binary_scorer.id)
    )
...
def test_successful_authentication_and_string_scorer_id(
    scorer_api_key,
    scorer_community_with_binary_scorer,
    passport_holder_addresses,
    mocker,
    ...
    address = passport_holder_addresses[0]["address"].lower()
    event = make_test_event_scorer_id(
        scorer_api_key, address, str(scorer_community_with_binary_scorer.id)
    )
