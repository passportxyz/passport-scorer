...
class TestPassportGetStamps:
    ...
    def test_get_stamps_with_address_with_no_scores(
        ...
        client = Client()
        response = client.get(
            f"{self.base_url}/{passport_holder_addresses[0]['address']}",
    ...
    def test_get_stamps_only_includes_this_address(
        ...
        client = Client()
        response = client.get(
            f"{self.base_url}/{passport_holder_addresses[0]['address']}?limit={limit}",
            HTTP_AUTHORIZATION="Token " + scorer_api_key,
        )
    ...
    def test_include_metadata(
        ...
        with mocker.patch(
            ...
            client = Client()
            response = client.get(
                f"{self.base_url}/{passport_holder_addresses[0]['address']}?include_metadata=true&limit=1",
                HTTP_AUTHORIZATION="Token " + scorer_api_key,
            )
            response_data = response.json()
    ...
    def test_limit_greater_than_1000_throws_an_error(
        ...
        client = Client()
        response = client.get(
            f"{self.base_url}/{passport_holder_addresses[0]['address']}?limit=1001",
            HTTP_AUTHORIZATION="Token " + scorer_api_key,
        )
    ...
    def test_limit_of_1000_is_ok(self, passport_holder_addresses, scorer_api_key):
        client = Client()
        response = client.get(
            f"{self.base_url}/{passport_holder_addresses[0]['address']}?limit=1000",
            HTTP_AUTHORIZATION="Token " + scorer_api_key,
        )
